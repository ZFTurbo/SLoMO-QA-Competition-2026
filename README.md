...
Solution for [SLoMO-QA Competition](https://eval.ai/web/challenges/challenge-page/2713/overview)
...
Download data from [https://huggingface.co/datasets/rghermi/sf20k-qa/tree/main](https://huggingface.co/datasets/rghermi/sf20k-qa/tree/main)
...
If you put data (videos and questions) in `./input/` folder
...
### Run inference for main track

First you need to run vLLM in 1st console:
```bash
...
--allowed-local-media-path {your local path with dataset} \
...
After run main processing script:
```bash
python run_main_track_qwen38_27fp8_with_vllm.py ./input/
```

### Run inference for special track
