...
def proc_with_qwen_8b():
    questions = pd.read_csv(INPUT_PATH + '/sf20k_private_test_questions.csv')
    ...
    for f in files:
        ...
        transcript_path = INPUT_PATH + "/sf20k_private_test_audios_vocals/" + video_id + '/vocals_mono_text_segment.txt'
        transcript_text = ""
        ...
        for index, row in part.iterrows():
            ...
            cache_file = out_cache_folder + video_id + '_{}_qid_{}_results_{}_{}_char.txt'.format(
                TYPE, qid, N_FRAMES, model_name
            )

            process = True
            if os.path.isfile(cache_file):
                print("Restore from cache!")
                output_text = open(cache_file, 'r', encoding='utf8').read()
                process = False
